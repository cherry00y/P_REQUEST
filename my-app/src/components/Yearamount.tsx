...
const YearlyRequestsChart: React.FC = () => {
  ...
  const [isCustomYearSelected, setIsCustomYearSelected] = useState(false); // Added state for custom year
  ...
  const handleRangeChange = (range: string) => {
    ...
    if (range === 'Custom Year') {
      ...
      setCustomYear(''); // Clear customYear when not using Custom Year
  ...
  filteredData.forEach((item: DataPoint) => {
    ...
    if (item.request_type === 'Repair Request') {
      chartData.datasets[0].data[monthIndex] += item.count; // เพิ่มจำนวนตาม count ของแต่ละข้อมูล
    } else if (item.request_type === 'New Request') {
      chartData.datasets[1].data[monthIndex] += item.count; // เพิ่มจำนวนตาม count ของแต่ละข้อมูล
    }
  });
