...
export default function Cost(){
    ...
    const [repairlogId, setRepairlogId] = useState<string | null>(null);
    ...
    useEffect(() => {
        const storedRepairlogId = localStorage.getItem('repairlog_id');
        if (storedRepairlogId) {
            setRepairlogId(storedRepairlogId);
        } else {
    ...
    const handleAccept = async () => {
        ...
        const repairlog_id = queryParams.get('repairlog_id'); // Fetch newrequest_id from URL
        ...
        // Prepare cost data for submission
        const costData = {
            repairlog_id, // Send newrequest_id from URL
            orderItems: orderItems.map(item => ({
                productname: item.productname, // Ensure this is an ID if required
                price: item.price,
                quantity: item.quantity, // Set order based on the index
            })),
        };
